...
public:
	...
	typedef void (*edge_visitor)(ARGraph_impl *g, node_id n1, node_id n2,
			void *attr, param_type param);
...
public:
	...
	typedef void *param_type;
	typedef void (*edge_visitor)(ARGraph *g, node_id n1, node_id n2, Edge *attr,
			param_type param);
	...
	Edge *GetEdgeAttr(node_id n1, node_id n2) {
		if (ARGraph_impl::GetEdgeAttr(n1, n2) == NULL) {
			return NULL;
		}
	...
	void VisitEdges(node_id node, edge_visitor vis, param_type param) {
		ARGraph_impl::VisitEdges(node, (ARGraph_impl::edge_visitor) vis, param);
	}
 ...
 * Also gets the attribute of the edge
 -----------------------------------------------*/
inline node_id ARGraph_impl::GetInEdge(node_id node, int i, void **pattr) {
	assert(node < n);
	assert(i < in_count[node]);
 ...
 * Also gets the attribute of the edge
 -----------------------------------------------*/
inline node_id ARGraph_impl::GetOutEdge(node_id node, int i, void **pattr) {
	assert(node < n);
	assert(i < out_count[node]);
