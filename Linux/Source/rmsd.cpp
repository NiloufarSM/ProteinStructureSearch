 ...
 */
void setup_rotation(float** ref_xlist, float** mov_xlist, int n_list,
		float mov_com[3], float mov_to_ref[3], float R[3][3], float* E0) {
	int i, j, n;
	float ref_com[3];
 ...
 */
int diagonalize_symmetric(float matrix[3][3], float eigen_vec[3][3],
		float eigenval[3]) {
	int n_rot, i, j, k;
	float vec[3][3];
	float val;
 ...
 */
int calculate_rotation_matrix(float R[3][3], float U[3][3], float E0,
		float* residual) {
	int i, j, k;
...
void calculate_rotation_rmsd(float** ref_xlist, float** mov_xlist, int n_list,
		float mov_com[3], float mov_to_ref[3], float U[3][3], float* rmsd) {
	float Eo, residual;
	float R[3][3];
...
void fast_rmsd(float **ref_xlist, float **mov_xlist, int n_list, float* rmsd) {
	float R[3][3];
	float d0, d1, d2, e0, e1, f0;
	float omega;
	float mov_com[3];
	float mov_to_ref[3];
